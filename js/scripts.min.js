...
$(function() {
	...
	,curPercentBG
	//,scrollW
	//,scrollMouseSpeed = 100
	//,timeScrolliAmination = 500
	//,ended=true
	//,dir
	//,numberBlock
	//,isAnimation = false
	...
	function start() {
		...
		specialtiesItems = $('.specialties__item');
		blocks = $('.block');
		$(blocks[0]).addClass('block__1');
		nBlocks = blocks.length;
		percentBG = $('.bg').css('transform');
		console.log("percentBG", percentBG);
		percentBG = percentBG.split(',')[5];
		curPercentBG = parseFloat(percentBG);
		percentBG = 2*curPercentBG/(nBlocks-1);
		console.log("percentBG", percentBG);
		//mbGrid = $(specialtiesItems).outerHeight(true) - $(specialtiesItems).outerHeight();
		//var itemWidth = $('.grid-sizer').outerWidth(true);
		//itemWidth = $('.specialties__item').outerWidth(true);
	}

	function getBlocks() {
		for (var j = 0; j < nBlocks; j++) {
			var tempEl = $(blocks[j]);
			var heightBlock = $(tempEl).find('.block__content').height();
			var top = tempEl.offset().top;
			//var left = offset.left;
			//var right = left + link.outerWidth();
			var isBig = heightBlock > (wh - 112*2 ) ? true : false;
			if (!isBig) {
				$(tempEl).css({'height': '100vh'});
			} else {
				$(tempEl).css({'height': 'auto','padding-top':'8rem','padding-bottom':'8rem'});
				if (j === nBlocks-1) {
					$(tempEl).css({'padding-bottom':'3rem'});
					$(tempEl).append('<div class="copyright">Copyright © 2017 Waxdesign.ru. All rights reserved.</div>');
				}
			}
			var bottom = top + tempEl.outerHeight();
			arrObjects[j] = {
				id: j,
				height: heightBlock,
				top: top,
			...
			console.log($(tempEl), heightBlock, top, bottom);
		}
	}
	...
	function firstAnimation() {
		$('.block__1 .block__content').addClass('loaded').addClass('changedIn');
	}

	function scrollToAnimate(el, time, fn) {
		$('html,body').stop().animate({
			scrollTop: el
		}, time, runOnce(function() {
			console.log('scroll complete');
			fn();
		}));
	}

	// function handScroll(up) {
	// 	isAnimation = true;
	// 	//var up = (dir === 'up') ? 1 : -1;
	// 	console.log("up", up, ' numberBlock', numberBlock);
	// 	if (up === 1) {
	// 		if (arrObjects[numberBlock-2].isBig) {
	// 			scrollToAnimate(arrObjects[numberBlock-2].bottom - wh, timeScrolliAmination, function() {
	// 				isAnimation = false;
	// 			});
	// 		} else {
	// 			scrollToAnimate(arrObjects[numberBlock-2].top, timeScrolliAmination, function() {
	// 				isAnimation = false;
	// 			});
	// 		}
	// 	} else {
	// 		scrollToAnimate(arrObjects[numberBlock].top, timeScrolliAmination, function() {
	// 			isAnimation = false;
	// 		});
	// 	}
	// 	$('.bg').removeClass('changed2').removeClass('changed3').removeClass('changed4');
	// 	$('.bg').addClass('changed'+(numberBlock-up));
	// 	$(blocks[numberBlock-1]).find('.block__content').removeClass('changedIn');
	// 	$(blocks[numberBlock-up-1]).find('.block__content').addClass('changedIn');
	// }

	// function isHandScroll(dir) {
	// 	var up = (dir === 'up') ? 1 : -1;
	// 	if(ended){
	// 		if ( ((up === 1) && (numberBlock > 1)) || ((up === -1) && (numberBlock < arrObjects.length)) ) {
	// 			handScroll(up);
	// 		}
	// 	} else {
	// 		// with normal scrollling
	// 		$('html,body').scrollTop(scrollW - up*scrollMouseSpeed);
	// 		scrollW = $(window).scrollTop();
	// 		if ( (up === 1) && (arrObjects[numberBlock-1].top - scrollW >= 3*wh/10) ) {
	// 			ended = true;
	// 			if (numberBlock > 1) {
	// 				handScroll(up);
	// 			}
	// 		}
	// 		if ( (up === -1) && (arrObjects[numberBlock-1].bottom - scrollW <= 7*wh/10 )) {
	// 			ended = true;
	// 			if (numberBlock < arrObjects.length) {
	// 				handScroll(up);
	// 			}
	// 		}
	// 	}
	...
	function validate(el) {
		...
		if ( !$(el).hasClass('questions__form') ) {
			console.log(el);
			var myInputs = $(el).find("input:not([type='submit']), textarea, select");
		} else {
			var myInputs = $(el).find("input:not([type='submit']):not([name='phone']), textarea, select");
		}
		...
		if (inputsWithValues == myInputs.length) {
			...
			$(el).find('.required__submit').css({'display':'none'});
		} else {
			$(el).find("input[type='submit']").prop("disabled", true);
			$(el).find('.required__submit').css({'display':'block'});
		}
	}
	...
	$('form').each( function() {
		var form = $(this);
		$(this).find("input:not([type='submit']), textarea, select").bind('change keyup', function() {
			validate(form);
		});
	});
	...
	$('.map').hover(
		...
		console.log($.fn.fullpage);
			$('.fp-scroller').css({'pointer-events':'none'});
		...
		console.log('unhover');
		console.log($.fn.fullpage);
			$('.fp-scroller').css({'pointer-events':'all'});
	...
	//$(document).bind('load', function() {
		if (ww > 767) {
			$('.menu a').addClass('changed1');
			$('.menu a').bind('transitionend', function () {
				$('.menu a').addClass('changed2');
			});
		}
		scrollToAnimate($('html,body').offset().top, 100, firstAnimation);
	//$(document).ready(function() {
		$('.content').fullpage({
			scrollOverflow: true,
			...
			scrollOverflowOptions: {
				...
				mouseWheel: false,
				disableMouse: true,
				disablePointer: (!isTouchDevice) ? true : false,
				disableTouch: true,
				hideScrollbars: true,
			...
			onLeave: function(index, nextIndex, direction){
				//if (!isAnimation) {
					...
					// if(ended) {
					// 	for (var j = 0; j < nBlocks; j++) {
					// 		if (scrollW >= Math.floor(arrObjects[j].top) && scrollW <= Math.floor(arrObjects[j].bottom) ) {
					// 			numberBlock = j+1;
					// 		}
					// 	}
					// 	if (arrObjects[numberBlock-1].isBig)
					// 		ended = false;
					// 	console.log('numberBlock = ', numberBlock);
					// }
				//}
				var up = (direction === 'up') ? 1 : -1;
				console.log("index", index, ' up', up);
				if (index === 1) {
					$('.bg,.bg__2').addClass('changed');
				}
				if ( (index === 2) && (up === 1) ) {
					$('.bg,.bg__2').removeClass('changed');
				}
				//$('.bg,.bg__2').css({'transform':'matrix(1.3, 0, 0, 1.3, 0,'+ (curPercentBG+percentBG*up) +')'});
				var animation = setInterval( function() {
					k++;
					console.log(k);
					$('.bg,.bg__2').css({'transform':'matrix(1.3, 0, 0, 1.3, 0,'+ (curPercentBG+percentBG*up*k/100) +')'});
				},5);
				setTimeout( function() {
					clearInterval(animation);
					curPercentBG += percentBG*up;
					k=0;
				}, 500);
				//$('.bg').removeClass('changed2').removeClass('changed3').removeClass('changed4');
				//$('.bg').addClass('changed'+(index-up));
				$(blocks[index-1]).find('.block__content').removeClass('changedIn');
				$(blocks[index-up-1]).find('.block__content').addClass('changedIn');
			},
			afterRender: function(){
				// GOOGLE MAPS HERE UPLOAD
				//initialization();
				if(!isTouchDevice){
					//$.fn.fullpage.options.disablePointer = false;
					//$.fn.fullpage.removeTouchHandler();
				}
			}
		});
	...
	$('.required__submit').on("click", function() {
		var text = $(this).parent().parent().find('.required__text');
	...
	$(window).bind('resize',function() {
		...
		/* masonry
			...
			specialtiesItems = $('.specialties__item');
			blocks = $('.block');
			//itemWidth = $('.grid-sizer').outerWidth(true);
			itemWidth = $('.specialties__item').outerWidth(true);
			$('body,html').stop().animate({
				scrollTop: $('body').offset().top
			}, 100, , runOnce(function() {
			onsole.log('scroll complete');
				$('.block__1 .block__content').addClass('loaded').addClass('changedIn');
			});
			if (ww > 767 ) {
			...
			if (ww > 575) {
				$('.specialties').masonry({
					...
					itemSelector: '.specialties__item',
					percentPosition: true,
					gutter: 0
				});
				setTimeout( function () {
					$('.specialties').masonry('reloadItems');
					$('.specialties').masonry({
						columnWidth: itemWidth,
						itemSelector: '.specialties__item',
						percentPosition: true,
	...
	/* masonry
		if (ww > 575) {
			$('.specialties').masonry({
				columnWidth: itemWidth,
				itemSelector: '.specialties__item',
				//columnWidth: $('.specialties').width()/2 - 6,
				percentPosition: true,
				gutter: 0
			});
		}
	*/
	if (ww <= 767) {
		$('body').append( $('.header__left>.menu') );
	}
	$('.hamburger').on("click", function() {
		//console.log("hamburger");
		$(this).toggleClass("is-active");
		$('.menu a').toggleClass('changed1');
		setTimeout( function() {
			$('.menu a').toggleClass('changed2');
		}, 250);
		$('.menu').toggleClass("changed");
	});
	$('.menu__categories-item').on("click", function() {
		var index = $('.menu__categories-item').index(this);
		$('.specialties').removeClass('active');
		$('.menu__categories-item').removeClass('active');
		$(this).addClass('active');
		$($('.specialties')[index]).addClass('active');
	});

	$('.reservation__form').submit(function(event) { //устанавливаем событие отправки для формы с id=form
		event.stopPropagation(); // Остановка происходящего
		event.preventDefault();  // Полная остановка происходящего
		//var id = $(this).attr('id');
		// let i = +$(this).attr('id').slice(-2);
		// if( isNaN(i) ) {
		// 	i = +$(this).attr('id').slice(-1);
		// }
		// if ($("#phone"+i).val() == '' && $("#email"+i).val() == '')
		// {
		// 	alert("Введите телефон или E-mail");
		// 	return false;
		// }
		// if ( (i!== 0) && (!$("#checkBox"+i).prop('checked')) ) {
		// 	alert("Прочитайте и согласитесь с политикой конфиденциальности");
		// 	return false;
		// }
		// console.log("Номер формы", i);
		var $that = $(this),
		formData = new FormData($that.get(0));
		//присоединяем наш файл
		// if( i == 1 ) {
		// 	var name = "#obrashenie__inputFiles"+i;
		// 	if ($(name)[0].files.length !== 0){
		// 		formData.append(name, $(name)[0].files[0]);	
		// 	}
		// 	console.log("name", name, "Кол-во файлов", $(name)[0].files.length);
		// }
		// console.log($(name)[0]);
		// console.log($(name)[0].files);
		// console.log($(name)[0].files.length);
		// console.log($(name)[0].files[0]);
		// formData.append('i', i);
		$.ajax({
			type: "POST", //Метод отправки
			url: "/api/sendFile.php", //путь до php фаила отправителя
			contentType: false, // важно - убираем форматирование данных по умолчанию
			processData: false, // важно - убираем преобразование строк по умолчанию
			data: formData,
			dataType: 'json',
			complete:function(data2) {
				var data2 = $.parseJSON(JSON.stringify(data2["responseText"])).slice(-3,-2);
				console.log(data2);
				if(data2 == "Y"){
					$('#obrashenie__form'+i).empty();
					$('#obrashenie__form'+i).append('<div class="newotvet"> Сообщение успешно отправлено!<br>В ближайшее время мы свяжемся с Вами!</div>');
					// $('.otvet').append('<p style="color: green !important; margin: 20px 0;">Сообщение успешно отправлено!<br>В ближайшее время мы свяжемся с Вами!</p>');
					// setTimeout(function() {
					// 	$that.trigger("reset");
					// 	$('.otvet').empty();
					// }, 1000);
				}else{
					$('.otvet').append('<p style="color: red !important; margin: 20px 0;">Сообщение не может быть отправлено!<br>Попробуйте позже!</p>');
					setTimeout(function() {
						$('.otvet').empty();
					}, 2000);
				}
	...
	getBlocks();
	for (var j = 0; j < nBlocks; j++) {
		console.log(arrObjects[j].id, arrObjects[j].height, arrObjects[j].top, arrObjects[j].bottom, arrObjects[j].isBig);
	}
	...
	// 			// 	console.log('up');
	// 			// }
	// 			dir = (touchStart > touchEnd) ? 'down' : 'up';
	...
	// var scrollWPrev = $(window).scrollTop();
	...
	// 		console.log("scrollW", scrollW);
	// 		var sY;
	// 		//if (e.originalEvent.detail != 0 && e.originalEvent.detail !== undefined) {
	// 			if (e.originalEvent) {
	// 				if (e.originalEvent.detail) {
	// 					sY = e.originalEvent.detail;
	// 				}
	// 			//if (e.originalEvent.wheelDelta != 0 && e.originalEvent.wheelDelta !== undefined) {
	// 				if (e.originalEvent.wheelDelta) {
	// 					sY = e.originalEvent.wheelDelta/(-40);
	// 				}
	// 			//if (e.originalEvent.deltaY != 0 && e.originalEvent.deltaY !== undefined) {
	// 				if (e.originalEvent.deltaY) {
	// 					sY = -e.originalEvent.deltaY;
	// 				}
	// 			}
	// 			console.log(sY,e.originalEvent.detail,e.originalEvent.wheelDelta,e.originalEvent.deltaY);
	// 			if (sY === undefined ) {
	// 				sY = ( scrollWPrev > scrollW) ? -1 :1;
	// 				scrollWPrev = scrollW;
	// 			}
	// 			dir = (sY <= 0) ? 'down' : 'up';
	// 			console.log( 'direction',dir);
	...
	// $(window).bind('DOMMouseScroll wheel mousewheel',function(e) {
	// 	if (!isAnimation) {
	// 		e.preventDefault();
	// 		scrollW = $(window).scrollTop();
	// 		console.log("scrollW", scrollW);
